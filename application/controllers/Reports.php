...
class Reports extends Secure_Controller
{
   ...
   //report by product 
    ...
    public function get_detailed_product_receivings_row($receiving_id)
	{
		$inputs = array('receiving_id' => $receiving_id);

		$this->load->model('reports/product_report');
		$model = $this->product_report;
    ...
    public function detailed_product_receivings($start_date, $end_date, $receiving_type, $location_id = 'all')
	{
		$inputs = array('start_date' => $start_date, 'end_date' => $end_date, 'receiving_type' => $receiving_type, 'location_id' => $location_id);
		...
		$this->load->view('reports/product_details', $data);
	}
    
    	public function product_summary_input()
	{
		$this->load->model('reports/Summary_product');
		$model = $this->Summary_product;
		...
		$this->load->view('reports/productlist', $data);
	}
    ...
    public function productlist()
    {
		...
		foreach($report_data as $row)
		{
			$tabular_data[] = $this->xss_clean(array(
				...
				'item_number' => $row['item_number'],
				'quantity' => to_quantity_decimals($row['Total_quty']),
				'reorder_level' => to_quantity_decimals($row['reorder_level']),
				'location_name' => $row['location_name'],
				'cost_price' => to_currency($row['cost_price']),
				'unit_price' => to_currency($row['unit_price']),
		...
		$this->load->view('reports/productreport', $data);
    }
    
    public function product_receivings()
	{
		$inputs = array('start_date' => $start_date, 'end_date' => $end_date, 'receiving_type' => $receiving_type, 'location_id' => $location_id);
